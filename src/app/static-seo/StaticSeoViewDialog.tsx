...
import {
  ...
  Box, Chip, TextField, Autocomplete, FormControl, InputLabel, Select, MenuItem, Divider
} from '@mui/material';
...
export default function StaticSeoViewDialog({
  open, onClose, seo, onSave, pageAccess, availableProductTags
    ...
    >
      <DialogTitle sx={{ 
        ...
        <Typography variant="h6" fontWeight={700}>
          {isEditMode ? '✏️ Editing' : '👁️ Viewing'}: {seo.name || 'SEO Entry'}
        </Typography>
        <IconButton 
          onClick={onClose}
          sx={{ 
            ...
            <Box sx={{ gridColumn: { xs: '1', sm: 'span 2', md: 'span 2', lg: 'span 2' } }}>
              {isEditMode ? (
                ...
                <Box>
                  ...
                  <Typography variant="body2" mt={0.5}>{currentData.name}</Typography>
                </Box>
              )}
            </Box>
                ...
                <Box>
                  ...
                  <Box mt={0.5}>
                    <Chip 
                      ...
                      size="small" 
                      color={currentData.status === 'published' ? 'success' : currentData.status === 'archived' ? 'default' : 'warning'}
                    />
                  </Box>
                </Box>
              )}
            ...
            <Box sx={{ gridColumn: { xs: '1', sm: 'span 2', md: 'span 3', lg: 'span 3' } }}>
              {isEditMode ? (
                ...
                <Box>
                  ...
                  <Typography variant="body2" mt={0.5}>{currentData.meta_title || 'N/A'}</Typography>
                </Box>
              )}
            </Box>
            ...
            <Box sx={{ gridColumn: { xs: '1', sm: 'span 2', md: 'span 3', lg: 'span 3' } }}>
              {isEditMode ? (
                <TextField
                  ...
                  onChange={(e) => setEditData({ ...editData, contentLanguage: e.target.value })}
                  fullWidth
                  placeholder="e.g., en"
                />
              ) : (
            ...
            <Box sx={{ gridColumn: { xs: '1', sm: 'span 2', md: 'span 3', lg: 'span 6' } }}>
              {isEditMode ? (
                ...
                <Box>
                  ...
                  <Typography variant="body2" mt={0.5}>{currentData.meta_description || 'N/A'}</Typography>
                </Box>
              )}
            </Box>
        ...
        <Box sx={{ mb: 3, p: 2, bgcolor: 'white', borderRadius: 2, boxShadow: 1 }}>
          ...
          {isEditMode ? (
            <Autocomplete
              ...
              value={Array.isArray(editData.producttag) ? editData.producttag : (editData.producttag ? [editData.producttag] : [])}
              onChange={(_, newValue) => setEditData({ ...editData, producttag: newValue })}
            ...
            <Box>
              ...
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5, mt: 0.5 }}>
                {Array.isArray(currentData.producttag) ? (
                  currentData.producttag.map((tag: string, i: number) => <Chip key={i} label={tag} size="small" />)
        ...
        <Box sx={{ mb: 3, p: 2, bgcolor: 'white', borderRadius: 2, boxShadow: 1 }}>
          ...
          {isEditMode ? (
            <TextField
              ...
              onChange={(e) => setEditData({ ...editData, description_html: e.target.value })}
              fullWidth
              multiline
              rows={4}
            />
            ...
            <Box>
              {currentData.description_html ? (
                <Box sx={{ p: 2, bgcolor: '#f5f5f5', borderRadius: 1, fontFamily: 'monospace', fontSize: '0.875rem', overflowX: 'auto' }}>
                  <pre style={{ margin: 0, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>{currentData.description_html}</pre>
                </Box>
              ) : (
                <Typography variant="body2">N/A</Typography>
              )}
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, 1fr)', md: 'repeat(3, 1fr)', lg: 'repeat(6, 1fr)' }, gap: 2 }}>
            <Box>
              {isEditMode ? (
                <TextField
                  ...
                  onChange={(e) => setEditData({ ...editData, ogLocale: e.target.value })}
                  fullWidth
            ...
            <Box>
              {isEditMode ? (
                <TextField
                  ...
                  onChange={(e) => setEditData({ ...editData, ogType: e.target.value })}
                  fullWidth
                  placeholder="e.g., website"
                />
              ) : (
                <Box>
                  <Typography variant="caption" color="text.secondary" fontWeight={600}>OG Type</Typography>
                  <Typography variant="body2" mt={0.5}>{currentData.ogType || 'N/A'}</Typography>
                </Box>
              )}
            </Box>
            ...
            <Box sx={{ gridColumn: { xs: '1', sm: 'span 2', md: 'span 1', lg: 'span 2' } }}>
              {isEditMode ? (
                <TextField
                  ...
                  onChange={(e) => setEditData({ ...editData, twitterCard: e.target.value })}
                  fullWidth
                  placeholder="e.g., summary_large_image"
                />
              ) : (
                <Box>
                  <Typography variant="caption" color="text.secondary" fontWeight={600}>Twitter Card</Typography>
                  <Typography variant="body2" mt={0.5}>{currentData.twitterCard || 'N/A'}</Typography>
                </Box>
              )}
            </Box>
            ...
            <Box sx={{ gridColumn: { xs: '1', sm: 'span 2', md: 'span 3', lg: 'span 2' } }}>
              {isEditMode ? (
                ...
                <Box>
                  <Typography variant="caption" color="text.secondary" fontWeight={600}>OG Image URL</Typography>
                  <Typography variant="body2" mt={0.5} sx={{ wordBreak: 'break-all' }}>
                    {getNestedValue(currentData, 'openGraph.images[0]') || 'N/A'}
                  </Typography>
                </Box>
              )}
            </Box>
            ...
            <Box sx={{ gridColumn: { xs: '1', sm: 'span 2', md: 'span 3', lg: 'span 3' } }}>
              {isEditMode ? (
                <TextField
                  ...
                  onChange={(e) => setEditData({ ...editData, og_twitter_Title: e.target.value })}
                  fullWidth
                />
              ) : (
                <Box>
                  <Typography variant="caption" color="text.secondary" fontWeight={600}>OG/Twitter Title</Typography>
                  <Typography variant="body2" mt={0.5}>{currentData.og_twitter_Title || 'N/A'}</Typography>
                </Box>
              )}
            </Box>
            ...
            <Box sx={{ gridColumn: { xs: '1', sm: 'span 2', md: 'span 3', lg: 'span 3' } }}>
              {isEditMode ? (
                <TextField
                  ...
                  onChange={(e) => setEditData({ ...editData, og_twitter_Description: e.target.value })}
                  fullWidth
                />
              ) : (
                <Box>
                  <Typography variant="caption" color="text.secondary" fontWeight={600}>OG/Twitter Description</Typography>
                  <Typography variant="body2" mt={0.5}>{currentData.og_twitter_Description || 'N/A'}</Typography>
                </Box>
              )}
            </Box>
          </Box>
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, 1fr)', md: 'repeat(3, 1fr)', lg: 'repeat(6, 1fr)' }, gap: 2 }}>
            <Box sx={{ gridColumn: { xs: '1', sm: 'span 2', md: 'span 2', lg: 'span 2' } }}>
              {isEditMode ? (
                ...
                <Box>
                  <Typography variant="caption" color="text.secondary" fontWeight={600}>Video URL</Typography>
                  <Typography variant="body2" mt={0.5} sx={{ wordBreak: 'break-all' }}>
                    {getNestedValue(currentData, 'openGraph.video.url') || 'N/A'}
                  </Typography>
                </Box>
              )}
            </Box>
            ...
            <Box sx={{ gridColumn: { xs: '1', sm: 'span 2', md: 'span 2', lg: 'span 2' } }}>
              {isEditMode ? (
                <TextField
                  ...
                  value={getNestedValue(editData, 'openGraph.video.secure_url') || ''}
                  onChange={(e) => updateNestedField('openGraph.video.secure_url', e.target.value)}
                  fullWidth
                />
              ) : (
                <Box>
                  <Typography variant="caption" color="text.secondary" fontWeight={600}>Video Secure URL</Typography>
                  <Typography variant="body2" mt={0.5} sx={{ wordBreak: 'break-all' }}>
                    {getNestedValue(currentData, 'openGraph.video.secure_url') || 'N/A'}
                  </Typography>
                </Box>
              )}
            </Box>
            ...
            <Box>
              {isEditMode ? (
                <TextField
                  size="small"
                  label="Video Type"
                  value={getNestedValue(editData, 'openGraph.video.type') || ''}
                ...
                <Box>
                  <Typography variant="caption" color="text.secondary" fontWeight={600}>Video Type</Typography>
                  <Typography variant="body2" mt={0.5}>{getNestedValue(currentData, 'openGraph.video.type') || 'N/A'}</Typography>
                </Box>
              )}
            </Box>
            ...
            <Box>
              {isEditMode ? (
                <TextField
                  ...
                  label="Video Width"
                  type="number"
                  value={getNestedValue(editData, 'openGraph.video.width') || ''}
                  onChange={(e) => updateNestedField('openGraph.video.width', Number(e.target.value))}
                  fullWidth
                />
              ) : (
                <Box>
                  <Typography variant="caption" color="text.secondary" fontWeight={600}>Video Width</Typography>
                  <Typography variant="body2" mt={0.5}>{getNestedValue(currentData, 'openGraph.video.width') || 'N/A'}</Typography>
                </Box>
              )}
            </Box>
          </Box>
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, 1fr)', md: 'repeat(3, 1fr)', lg: 'repeat(6, 1fr)' }, gap: 2 }}>
            <Box sx={{ gridColumn: { xs: '1', sm: 'span 2', md: 'span 2', lg: 'span 3' } }}>
              {isEditMode ? (
                ...
                <Box>
                  <Typography variant="caption" color="text.secondary" fontWeight={600}>Twitter Image URL</Typography>
                  <Typography variant="body2" mt={0.5} sx={{ wordBreak: 'break-all' }}>
        ...
        <Box sx={{ p: 2, bgcolor: 'white', borderRadius: 2, boxShadow: 1 }}>
          ...
          {isEditMode ? (
            <TextField
              ...
              onChange={(e) => setEditData({ ...editData, VideoJsonLd: e.target.value })}
              fullWidth
              multiline
              rows={4}
            />
            ...
            <Box>
              {currentData.VideoJsonLd ? (
                <Box sx={{ p: 2, bgcolor: '#f5f5f5', borderRadius: 1, fontFamily: 'monospace', fontSize: '0.875rem', overflowX: 'auto' }}>
                  <pre style={{ margin: 0, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>{currentData.VideoJsonLd}</pre>
                </Box>
              ) : (
                <Typography variant="body2">N/A</Typography>
              )}
